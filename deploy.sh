...
# 用法:./deploy.sh
#
# 智能跳过:如果 scripts/scenes/audio/... 都没改过,跳过 export 步骤,
...
if [ ! -f docs/index.pck ] || [ ! -f docs/index.wasm ]; then
  ...
  NEWER=$(find scripts scenes audio fonts models themes project.godot \
...
if [ "$NEED_EXPORT" = "1" ]; then
  ...
  # 导致源码改了但 export 出来没生效(OVERNIGHT_LOG.md 踩过这个坑)
...
# `godot --import` 重建 .godot/imported/ 缓存(新加的字体/贴图/模型才能加载)
# `sudo systemctl restart` 对 root 是空操作,对 kid 用户走 sudoers 白名单
echo ""
