# Server Architecture & One-Time Setup

> 📘 English version first. 中文版在文件下半部分 ([跳到中文](#服务器架构与一次性搭建指南中文)).

For day-to-day operations, see [OPERATIONS.md](OPERATIONS.md).
This document describes **the current architecture** and **the complete zero-to-live setup** (useful when switching servers or domains).
...
```
Player (browser / phone)
   │
   ▼  https://game.boobank.com/...
[Cloudflare edge network] (Tokyo / HK / global PoPs)
   │
   ▼  Cloudflare Named Tunnel (QUIC, outbound only; no inbound ports needed)
[Vultr Tokyo Ubuntu 24.04 server] root@207.148.98.206
   │
   ├── cloudflared.service        ────► localhost:80
   │
   ├── caddy.service (port 80)
   │     ├── /                     ───► /opt/games/portal/ (portal static)
   │     ├── /arena-shooter/       ───► /opt/games/arena-shooter-3d/docs/ (game static)
   │     └── /arena-shooter/ws     ───► localhost:7777 (reverse-proxy to Godot)
   │
   └── arena-game.service          ────► Godot --headless, listens on :7777
                                          (WebSocketMultiplayerPeer)
```

**Key design choices**:
- **TLS terminates at Cloudflare's edge**, origin only speaks HTTP :80 → **no cert management on the server**
- **Inbound: only SSH** (UFW 22/tcp). All player traffic comes through the cloudflared outbound connection.
- **Caddy handles same domain** for both static files and WebSocket (path-based + Upgrade header)
- **Multiple games are separated by path** (`/arena-shooter/`, `/{next-game}/`)
- **Code → Server** is git-only. The server is just a runtime mirror of git; **anything can be rebuilt from git**.

---

## One-time setup (run this once on a new server / new provider)

### 0. Prerequisites

- Domain (this project uses `boobank.com`) is on Cloudflare DNS
- A named tunnel `arena-shooter` exists in Cloudflare with `cloudflared tunnel route dns arena-shooter game.boobank.com`
- The Mac has `~/.cloudflared/cert.pem` + `<UUID>.json` (tunnel credentials)

If none of the above exists, do it on the Mac first:
```bash
brew install cloudflared
cloudflared tunnel login                               # browser picks boobank.com
cloudflared tunnel create arena-shooter                # generates UUID + JSON
cloudflared tunnel route dns arena-shooter game.boobank.com
```

### 1. Spin up a VPS

Any Linux VPS works. This project uses **Vultr Tokyo Cloud Compute Shared CPU $12/mo**:
- 2GB RAM / 1 vCPU / 55GB SSD / 2TB traffic
- Ubuntu 24.04 LTS x64
- Attach SSH key

### 2. SSH in + system bootstrap
...
# Firewall: only allow SSH. Game traffic comes through cloudflared (outbound),
# so no inbound ports needed.
...
### 4. Copy the tunnel credentials from Mac to the server
...
**Back on the server**, write the cloudflared config:
```yaml
...
```bash
cloudflared --config /etc/cloudflared/config.yml tunnel ingress validate   # should print OK
```

### 5. Install Godot 4.6.2 headless
...
godot --version    # should print 4.6.2.stable.official...
```
...
godot --headless --path . --import     # generates .godot/imported/* cache
```
...
```bash
caddy validate --config /etc/caddy/Caddyfile     # should print Valid configuration
```
...
cloudflared has a built-in systemd unit generator:
```bash
...
sleep 4
systemctl is-active arena-game caddy cloudflared       # all three should be active
```

### 10. Verify

```bash
# From the Mac (or anywhere)
curl -I https://game.boobank.com/                            # 200, portal
curl -I https://game.boobank.com/arena-shooter/              # 200, game page
curl -s https://game.boobank.com/arena-shooter/server.json   # {"url": "wss://..."}

# WSS handshake (should reply 101 Switching Protocols)
python3 -c "
...
Open https://game.boobank.com/arena-shooter/ in a browser → pick a character → Join → you're live!

---

## Adding a new game to this architecture

See the "Adding a new game" section of [OPERATIONS.md](OPERATIONS.md). In short:

1. Clone the new repo to `/opt/games/<game>/`
2. Add a `handle_path /<game>/*` block to Caddy
3. If it's multiplayer, also add `handle /<game>/ws { reverse_proxy localhost:<port> }` and a matching systemd unit

---

## Troubleshooting

See the "Troubleshooting" section in [OPERATIONS.md](OPERATIONS.md).

---

<a id="服务器架构与一次性搭建指南中文"></a>
...
git clone https://github.com/longmaolab/portal.git /opt/games/portal
cd /opt/games/arena-shooter-3d
godot --headless --path . --import     # 生成 .godot/imported/* 缓存
```
